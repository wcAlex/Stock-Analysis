...
def get_history_data(symbol, crypto_symbol, combine_with_crypto=True):
    ...
    stock_hourly_data = r.stocks.get_stock_historicals(
        symbol, interval="hour", span="3month")
    stock_hourly_dataframe = pd.DataFrame(stock_hourly_data)

    stock_daily_data = r.stocks.get_stock_historicals(
    ...
    stock_5min_data = r.stocks.get_stock_historicals(
    ...
    stock_hourly_dataframe.to_csv("{0}/{1}_hourly_3months_{2}.csv".format(
        raw_data_dir, symbol, date.today().strftime("%d_%m_%Y")), index=False)
    stock_daily_dataframe.to_csv("{0}/{1}_daily_1year_{2}.csv".format(
        raw_data_dir, symbol, date.today().strftime("%d_%m_%Y")), index=False)
    stock_5min_dataframe.to_csv("{0}/{1}_5min_1week_{2}.csv".format(
        raw_data_dir, symbol, date.today().strftime("%d_%m_%Y")), index=False)
    ...
    if combine_with_crypto:
        ...
        crypto_hourly_data = r.crypto.get_crypto_historicals(
            crypto_symbol, interval="hour", span="3month")
        crypto_hourly_dataframe = pd.DataFrame(crypto_hourly_data)

        crypto_daily_data = r.crypto.get_crypto_historicals(
            crypto_symbol, interval="day", span="year")
        crypto_daily_dataframe = pd.DataFrame(crypto_daily_data)

        crypto_5min_data = r.crypto.get_crypto_historicals(
            crypto_symbol, interval="5minute", span="week")
        ...
        crypto_stock_combined_hourly_dataframe = pd.merge(
            crypto_hourly_dataframe, stock_hourly_dataframe, on="begins_at", how="left")
        crypto_stock_combined_daily_dataframe = pd.merge(
            crypto_daily_dataframe, stock_daily_dataframe, on="begins_at", how="left")
        crypto_stock_combined_5min_dataframe = pd.merge(
            crypto_5min_dataframe, stock_5min_dataframe, on="begins_at", how="left")
        ...
        crypto_stock_combined_5min_dataframe.to_csv("{0}/{1}_{2}_5min_weekly_combined_{3}.csv".format(
            data_dir, crypto_symbol, symbol, date.today().strftime("%d_%m_%Y")), index=False)
        crypto_stock_combined_hourly_dataframe.to_csv("{0}/{1}_{2}_hour_3months_combined_{3}.csv".format(
            data_dir, crypto_symbol, symbol, date.today().strftime("%d_%m_%Y")), index=False)
        crypto_stock_combined_daily_dataframe.to_csv("{0}/{1}_{2}_daily_year_combined_{3}.csv".format(
            data_dir, crypto_symbol, symbol, date.today().strftime("%d_%m_%Y")), index=False)
